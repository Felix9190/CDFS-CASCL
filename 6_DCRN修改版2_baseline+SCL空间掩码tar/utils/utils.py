...
# 按类别画点：这种覆盖关系单一，因为覆盖关系是根据plot的调用顺序决定的，这样就是按着类别出现的顺序覆盖，而不是随机测试的顺序覆盖
# def plot_embedding_2D(data, label, title, color_map):
#     x_min, x_max = np.min(data, 0), np.max(data, 0)
#     # data = (data - x_min) / (x_max - x_min)  # 去掉，和彭老师保持一致
#     fig = plt.figure()
#     unique_labels = np.unique(label)  # 获取唯一的标签
#
#     for lbl in unique_labels:
#         indices = np.where(label == lbl)  # 获取具有相同标签的数据点的索引
#         plt.plot(data[indices, 0], data[indices, 1], marker='o', color=color_map[lbl], label=lbl)
#
#     plt.title(title)
#     plt.legend()  # 添加图例
#     return fig
