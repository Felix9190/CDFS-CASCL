...
def get_train_test_loader(Data_Band_Scaler, GroundTruth, class_num, tar_lsample_num_per_class, shot_num_per_class, HalfWidth):
    ...
    imdb['Labels'] = imdb['Labels'] - 1 # 故IP就是0~15
...
def get_target_dataset(Data_Band_Scaler, GroundTruth, class_num, tar_lsample_num_per_class, shot_num_per_class, patch_size):
    ...
    target_da_train_set = {}
    target_aug_data_ssl = [] # target aug dataset. loader包含相同样本的增强
    target_aug_label_ssl = [] # target aug dataset. loader包含相同样本的增强
    ...
    # # 用于SSL的数据：增强后的所有数据按N*labels的顺序重排，然后loader也这个顺序迭代
    # target_aug_data_ssl = []
    # # 0-39 增强倍数=40：相对于原数据增强了多少倍，即能迭代多少组,一组 class_num * nlabeled，loader也自适应调节batchsize大小吧
    # aug_times = math.ceil((200 - tar_lsample_num_per_class) / tar_lsample_num_per_class) + 1
    # for i in range(aug_times):
    #     for j in range(class_num):
    #         for k in range(tar_lsample_num_per_class):
    #             target_aug_data_ssl.append(target_da_datas[j * 200 + i * tar_lsample_num_per_class + k, :, :, :])
