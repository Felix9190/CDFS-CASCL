...
from utils.dataloader import get_HBKC_data_loader, Task, get_target_dataset_houston, get_target_dataset, getMetaTrainLabeledDataset, get_metatrain_Labeled_data_loader, tagetSSLDataset
...
seeds = [1222, 1230, 1333, 1337, 1339, 1231, 1233, 1234, 1212, 1213] # HT final seeds version
# seeds = [1234] # best classification map and t-SNE
...
for iDataSet in range(nDataSet) :
    ...
    for episode in range(EPISODE) :
        ...
        # 一次增强
        # augment1_support_proto_tar = torch.FloatTensor(data_augment.random_mask_batch_image(support_tar.data.cpu(), 0.9))  # (16, 200, 7, 7)
        # features_augment_tar = encoder(mapping_tar(augment1_support_proto_tar.to(GPU)))  # (32, 128)
        # scl_loss_tar = infoNCE_Loss_SCL(features_augment_tar, support_proto_tar)
        ...
        # #  两次增强
        # augment1_support_proto_tar = torch.FloatTensor(data_augment.random_mask_batch_image(support_tar.data.cpu(), 0.2))  # (16, 200, 7, 7)
        # augment2_support_proto_tar = torch.FloatTensor(data_augment.random_mask_batch_image(support_tar.data.cpu(), 0.2))  # (16, 200, 7, 7)
        ...
        # scl_loss_tar = infoNCE_Loss_SCL(features_augment[:TAR_CLASS_NUM, :], features_augment[TAR_CLASS_NUM:, :])
        ...
        augment1_target_ssl_data = torch.FloatTensor(data_augment.random_mask_batch_image(target_ssl_data.data.cpu(), 0.6))  # (128, 200, 7, 7)
        augment2_target_ssl_data = torch.FloatTensor(data_augment.random_mask_batch_image(target_ssl_data.data.cpu(), 0.6))  # (128, 200, 7, 7)
