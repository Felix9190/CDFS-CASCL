...
infoNCE_Loss = loss_function.ContrastiveLoss(batch_size = TAR_CLASS_NUM, temperature=0.2).cuda()
...
for iDataSet in range(nDataSet) :
    ...
    for episode in range(EPISODE) :
        ...
        # logits_src = F.cosine_similarity(query_features_src.unsqueeze(1), support_proto_src.unsqueeze(0), dim=-1)  # simi_mat: (2*bs, 2*bs)
        ...
        # logits_tar = F.cosine_similarity(query_features_tar.unsqueeze(1), support_proto_tar.unsqueeze(0), dim=-1)  # simi_mat: (2*bs, 2*bs)
        f_loss_tar = crossEntropy(logits_tar, query_label_tar.long().to(GPU))

        f_loss = f_loss_src + f_loss_tar

        # add align loss. align loss = image align + text align
        # image_align_loss = infoNCE_Loss(spatial_spectral_fusion_feature_src, support_features_src) + infoNCE_Loss(spatial_spectral_fusion_feature_tar, support_features_tar)
        ...
        # spatial_align_loss = infoNCE_Loss(spatial_feature_src, spatial_spectral_fusion_feature_src) + infoNCE_Loss(spatial_feature_tar, spatial_spectral_fusion_feature_tar)
        # spectral_align_loss = infoNCE_Loss(spectral_feature_src, spatial_spectral_fusion_feature_src) + infoNCE_Loss(spectral_feature_tar, spatial_spectral_fusion_feature_tar)

        # loss = f_loss + image_align_loss + 5 * text_align_loss + spatial_align_loss + spectral_align_loss
        loss = f_loss + 1.0 * text_align_loss
