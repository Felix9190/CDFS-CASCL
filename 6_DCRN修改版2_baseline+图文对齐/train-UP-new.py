...
infoNCE_Loss_SCL = loss_function.ContrastiveLoss(batch_size = TAR_CLASS_NUM).cuda()
...
for iDataSet in range(nDataSet) :
    ...
    for episode in range(EPISODE) :
        ...
        #  add target support SCL
        augment1_support_proto_tar = torch.FloatTensor(data_augment.Crop_and_resize_batch(support_tar.data.cpu(), patch_size // 2)) # (16, 200, 7, 7)
        augment2_support_proto_tar = torch.FloatTensor(data_augment.Crop_and_resize_batch(support_tar.data.cpu(), patch_size // 2)) # (16, 200, 7, 7)
